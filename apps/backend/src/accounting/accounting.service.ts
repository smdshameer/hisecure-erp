...
export class AccountingService {
    ...
    async exportSales(startDate: string, endDate: string) {
        ...
        // CSV Header
        let csv = 'Invoice No,Date,Customer,Total Amount,Payment Method,Sold By,Items\n';
        ...
        sales.forEach((sale) => {
            ...
            csv += `${sale.invoiceNo},${date},${customer},${sale.totalAmount},${sale.paymentMethod},${sale.user.name},${safeItems}\n`;
        });
    ...
    async exportPurchases(startDate: string, endDate: string) {
        ...
        pos.forEach((po) => {
            ...
            const items = po.items
                .map((i) => `${i.product.name} (${i.quantity} @ ${i.unitCost})`)
                .join('; ');

            const safeItems = `"${items}"`;

            csv += `${po.poNumber},${date},${po.supplier.name},${po.status},${po.totalAmount},${safeItems}\n`;
        });
