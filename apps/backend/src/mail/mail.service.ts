...
export class MailService {
    ...
    async sendLowStockAlert(to: string, productName: string, currentStock: number) {
        if (!this.transporter) {
            await this.createTransporter();
        }
    ...
    async sendDailyReport(to: string, stats: { date: string; totalSales: number; totalOrders: number }) {
        if (!this.transporter) {
            await this.createTransporter();
        }
